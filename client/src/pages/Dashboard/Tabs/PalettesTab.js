...
function PalettesTab() {
  ...
  useEffect(() => {
    ...
    appDispatch({
      type: SET_PALETTES,
      payload: data.Palettes.filter(
        (palette) => palette.userId === authService.getProfile().data._id
      ),
    });
        ...
        {loading ? (
          ...
          appState.palettes.map((palette) => (
            <ItemContainer
              ...
              onRemove={async (e) => {
                e.stopPropagation();
                deletePalette({ variables: { paletteId: palette._id } }).then(
                  (res) => {
            ...
            >
              <div className="flex h-full overflow-hidden flex-1 rounded-lg shadow-md relative">
                {palette.colors.map((color) => (
                  <ColorVariantButton key={color} currentColor={color} />
                ))}
              </div>
            </ItemContainer>
          ))
