...
function FontsTab({ style }) {
  ...
  useEffect(() => {
    ...
    appDispatch({
      type: SET_FONTS,
      payload: data.Fonts.filter(
        (font) => font.userId === authService.getProfile().data._id
      ),
    });
                  ...
                  <>
                      ...
                      >
                        <a
                          href={
                            googleFonts.find(
                              (fontItem) =>
                                fontItem.family === font.activeFontFamily
                            )?.files.regular
                          }
                    ...
                    >
                      <Button
                        ...
                        className="w-full flex justify-center items-center gap-1"
                        onClick={() =>
                          CopyAndAlert({ content: font.activeFontFamily })
                        }
                      >
                        Copy Font Family
                      </Button>
