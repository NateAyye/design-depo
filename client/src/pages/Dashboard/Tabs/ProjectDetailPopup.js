...
import { DELETE_PALETTE,REMOVE_ITEM_FROM_PROJECT } from "../../../lib/mutations"
...
import { REMOVE_COLOR, SET_COLORS ,REMOVE_FONT,REMOVE_GRADIENT} from "../../../context/AppState/actions"
...
function ProjectDetailPopup({ project, onClose }) {
    ...
    const [{ colors, colorFormat }] = useAppContext()
  ...
  return (
    <div >
        <div className="header_container" style={{ height: '200px' }}>
            <h1 className="content_h1">The Content of Project: {project.projectName}</h1>
            <nav>
                <ItemGrid >
                    <Button onClick={() => setActiveGrid("pallets")}
                    style={isButtonActive("pallets") ? activeButtonStyle:{}}>Pallets</Button>
                
                    <Button onClick={() => setActiveGrid("colors")}
                    style={isButtonActive("colors") ? activeButtonStyle:{}}>Colors</Button>
                
                    <Button onClick={() => setActiveGrid("fonts")}
                    style={isButtonActive("fonts") ? activeButtonStyle:{}}>Fonts</Button>
                
                    <Button onClick={() => setActiveGrid("gradients")}
                    style={isButtonActive("gradients") ? activeButtonStyle:{}}>Gradients</Button>
                </ItemGrid>
            </nav>
        </div>
      ...
      <div >
                    ...
                    {project.palettes.length > 0 ?(
                        project.palettes.map((palette) => (
                            <div>
                                ...
                                // console.log(project._id);
                                    deleteItem({ variables: {id:project._id, itemId: palette._id,type:"palettes" } }).then(res => {
                        ...
                        <div>
                            <ItemContainer
                                ...
                                // TODO: Add remove color functionality
                                    ...
                                    deleteItem({ variables: {id:project._id, itemId: color._id,type:"colors" } }).then((res) => {
                        ...
                        <div>
                            ...
                            <ItemContainer
                                ...
                                onRemove={async (e) => {
                                    e.stopPropagation()
                                    // TODO: Add remove color functionality
                                    deleteItem({ variables: {id:project._id, itemId: font._id,type:"fonts" } }).then((res) => {
                    ...
                    project.gradients.map((gradient) => (
                        <div>
                            <ItemContainer
                                ...
                                onRemove={async (e) => {
                                e.stopPropagation()
                                deleteItem({ variables: {id:project._id, itemId: gradient._id,type:"gradients" } }).then((res) => {
        ...
        <hr></hr>
        <Button onClick={onClose} className="content_Button">Go Back to projects list</Button>
      </div>
    </div>
  );
}
