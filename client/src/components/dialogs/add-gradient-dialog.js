...
import {
  ADD_GRADIENT,
  SET_TAB,
  UPDATE_GRADIENT as UPDATE_GRADIENT_STATE,
...
function AddGradientDialog({
  ...
  async function onFormSubmit(values) {
    ...
    try {
      const mutationResponse = editing
        ? await updateGradient({
            variables: { id: gradient._id, ...gradientValues },
          })
        : await createGradient({ variables: gradientValues });
      const gradientRes = mutationResponse.data;
      if (editing) {
        appDispatch({
          type: UPDATE_GRADIENT_STATE,
          payload: gradientRes.updateGradient,
        });
      } else {
        appDispatch({
          type: ADD_GRADIENT,
          payload: gradientRes.createGradient,
        });
      }
      toast({
  ...
  const onError = async (data) => {
    const message =
      data?.name?.message || data?.color?.message || 'Something went wrong.';
    toast({
      title: `Error: ${message}`,
      description:
        'Failed to save gradient. Please check all form fields or try again later. \n Thank you.',
      variant: 'destructive',
    });
  };
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <div className="relative">
                        ...
                        {displayColorPicker ? (
                          <div className="absolute left-1/2 -translate-x-1/2 -bottom-10 w-fit lg:left-full lg:-translate-x-0 lg:bottom-0 lg:translate-y-1/2 bg-white p-2 z-10">
                            <ColorPicker
                              hideColorTypeBtns
                              hidePresets
                              height={150}
                    ...
                    <FormDescription>Color Value</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <div
                className="min-h-[70px] rounded-lg overflow-hidden flex items-stretch"
